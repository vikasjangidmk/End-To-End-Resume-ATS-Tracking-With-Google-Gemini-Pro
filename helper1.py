...
def sanitize_text(text):
    """Sanitize text by removing invalid control characters and unwanted HTML."""
    
    # Step 1: Remove non-printable characters (except for newlines, tabs, and carriage returns)
    text = re.sub(r'[^\x20-\x7E\n\t\r]', '', text)
    
    # Step 2: Remove unwanted characters such as non-ASCII characters
    text = re.sub(r'[^\x00-\x7F]+', ' ', text)  # This replaces non-ASCII characters with space
    
    # Step 3: Remove extra whitespaces and clean up
    text = re.sub(r'\s+', ' ', text)  # Replace multiple whitespaces with a single space
    
    # Step 4: Use BeautifulSoup to remove any HTML tags (if any)
    text = BeautifulSoup(text, "html.parser").get_text()

    return text

def extract_pdf_text(uploaded_file):
    """Extract text from PDF with enhanced error handling and sanitization."""
    try:
        ...
        for page in reader.pages:
            ...
            if page_text:
                sanitized_text = sanitize_text(page_text)  # Sanitize the text
                text.append(sanitized_text)
